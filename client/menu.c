...
static void main_menu(void) {
  static struct {
    ...
    uint32_t level;
      ...
      {"Octane Queen-C", 1},
      {"Chenksopt Tlamtathat", 10},
      {"Tsimquuj SamhainCrow", 8},
      {"Lucy Sadee", 89},
  };
  ...
  for(uint32_t i = 0; i < sizeof(characters) / sizeof(characters[0]); i++) {
    // UI_Padding(0, 0, 4, 4);
    UI_Vertical();
    UI_Text("name:  %s", characters[i].name);
    UI_Text("level: %i", characters[i].name);
    UI_End();
  }
...
static void (*current_menu)(void) = null_func;
...
static void ResetDefaults(void *unused) { VID_MenuInit(); }
...
*/
void VID_MenuInit(void) {
  static const char *resolutions[] = {"[320 240  ]",
                                      "[400 300  ]",
                                      "[512 384  ]",
                                      "[640 480  ]",
                                      "[800 600  ]",
                                      ...
                                      "[1600 1200]",
                                      0};
  static const char *refs[] = {"[software      ]", "[default OpenGL]", "[3Dfx OpenGL   ]", "[PowerVR OpenGL]",
                               //		"[Rendition OpenGL]",
                               0};
...
static void CrosshairFunc(void *unused) { Cvar_SetValue("crosshair", s_options_crosshair_box.curvalue); }

static void JoystickFunc(void *unused) { Cvar_SetValue("in_joystick", s_options_joystick_box.curvalue); }

static void CustomizeControlsFunc(void *unused) { M_Menu_Keys_f(); }

static void AlwaysRunFunc(void *unused) { Cvar_SetValue("cl_run", s_options_alwaysrun_box.curvalue); }

static void FreeLookFunc(void *unused) { Cvar_SetValue("freelook", s_options_freelook_box.curvalue); }

static void MouseSpeedFunc(void *unused) { Cvar_SetValue("sensitivity", s_options_sensitivity_slider.curvalue / 2.0F); }

static void NoAltTabFunc(void *unused) { Cvar_SetValue("win_noalttab", s_options_noalttab_box.curvalue); }
...
static void LookspringFunc(void *unused) { Cvar_SetValue("lookspring", s_options_lookspring_box.curvalue); }

static void LookstrafeFunc(void *unused) { Cvar_SetValue("lookstrafe", s_options_lookstrafe_box.curvalue); }

static void UpdateVolumeFunc(void *unused) { Cvar_SetValue("s_volume", s_options_sfxvolume_slider.curvalue / 10); }

static void UpdateCDVolumeFunc(void *unused) { Cvar_SetValue("cd_nocd", !s_options_cdvolume_box.curvalue); }
...
const char *Options_MenuKey(int key) { return Default_MenuKey(&s_options_menu, key); }
...
void M_Credits_MenuDraw(void) {
  ...
  ** draw the credits
  */
  for(i = 0, y = viddef.height - ((cls.realtime - credits_start_time) / 40.0F); credits[i] && y < viddef.height;
      y += 10, i++) {
...
const char *Game_MenuKey(int key) { return Default_MenuKey(&s_game_menu, key); }
...
const char *JoinServer_MenuKey(int key) { return Default_MenuKey(&s_joinserver_menu, key); }
...
const char *DMOptions_MenuKey(int key) { return Default_MenuKey(&s_dmoptions_menu, key); }
...
const char *DownloadOptions_MenuKey(int key) { return Default_MenuKey(&s_downloadoptions_menu, key); }
...
static void HandednessCallback(void *unused) { Cvar_SetValue("hand", s_player_handedness_box.curvalue); }
...
void PlayerConfig_MenuDraw(void) {
  ...
  if(s_pmi[s_player_model_box.curvalue].skindisplaynames) {
    ...
    M_DrawTextBox((refdef.x) * (320.0F / viddef.width) - 8, (viddef.height / 2) * (240.0F / viddef.height) - 77,
