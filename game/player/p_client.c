  ...
  while(1) {
    ...
    if(VectorLength(d) < 384) {
      if((!self->targetname) || Q_stricmp(self->targetname, spot->targetname) != 0) {
        ...
        // vtos(self->s.origin), self->targetname, spot->targetname);
...
void SP_info_player_coop(edict_t *self) {
  ...
  if((Q_stricmp(level.mapname, "jail2") == 0) || (Q_stricmp(level.mapname, "jail4") == 0) ||
     (Q_stricmp(level.mapname, "mine1") == 0) || (Q_stricmp(level.mapname, "mine2") == 0) ||
     (Q_stricmp(level.mapname, "mine3") == 0) || (Q_stricmp(level.mapname, "mine4") == 0) ||
     (Q_stricmp(level.mapname, "lab") == 0) || (Q_stricmp(level.mapname, "boss1") == 0) ||
     (Q_stricmp(level.mapname, "fact3") == 0) || (Q_stricmp(level.mapname, "biggun") == 0) ||
     (Q_stricmp(level.mapname, "space") == 0) || (Q_stricmp(level.mapname, "command") == 0) ||
...
void player_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point) {
  ...
  if(ent_read_health(self)->value < -40) { // gib
...
void SaveClientData(void) {
  ...
  for(i = 0; i < game.maxclients; i++) {
    ...
    if(!ent->inuse)
      continue;
    game.clients[i].pers.health = ent_read_health(ent)->value;
    game.clients[i].pers.max_health = cv_get(ent_read_max_health(ent));
...
float PlayersRangeFromSpot(edict_t *spot) {
  ...
  for(n = 1; n <= maxclients->value; n++) {
    ...
    if(ent_read_health(player)->value <= 0)
      continue;
...
void body_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point) {
  ...
  if(ent_read_health(self)->value < -40) {
    gi.sound(self, CHAN_BODY, gi.soundindex("misc/udeath.wav"), 1, ATTN_NORM, 0);
    for(n = 0; n < 4; n++)
      ThrowGib(self, "models/objects/gibs/sm_meat/tris.md2", damage, GIB_ORGANIC);
    self->s.origin[2] -= 48;
