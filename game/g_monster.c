...
void monster_fire_bullet(edict_t *self, vec3_t start, vec3_t dir, int damage, int kick, int hspread, int vspread,
  ...
  gi.multicast(self->s.cmodel_index, start, MULTICAST_PVS);
}
...
void AttackFinished(edict_t *self, float time) { self->monsterinfo.attack_finished = level.time + time; }
...
bool monster_start(edict_t *self) {
  ...
  self->air_finished = level.time + 12;
  self->use = monster_use;
  cv_initialize(ent_write_max_health(self), ent_read_health(self)->value);
